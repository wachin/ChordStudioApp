...
- `ChordStudioApp` in `MainActivity.kt` contains the primary Compose screen and its UI state. Text state is split into `loadedText` (the originally opened file, immutable while the file is open, used by the "Restaurar original" action), `sourceText` (the current transposition base, updated when the user edits manually), `displayedText` (the rendered/editable text), and `hasManualEdits` (toggled when a manual edit diverges from the loaded file). Semitone and enharmonic-preference changes always recompute `displayedText` from `sourceText`.
...
- Transposition always derives from the transposition base (`sourceText`) instead of repeatedly transposing already-rendered text. `loadedText` must stay untouched except when a new file is opened or the user explicitly restores it, so "Restaurar original" can always undo both transposition and manual edits. Preserve these invariants when editing controls or adding new transpose actions.
